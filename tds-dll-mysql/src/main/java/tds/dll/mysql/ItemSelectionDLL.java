...
public class ItemSelectionDLL extends AbstractDLL implements IItemSelectionDLL {
	private static Logger _logger = LoggerFactory
			.getLogger(ItemSelectionDLL.class);
	...
	public SingleDataResultSet AA_GetNextItemCandidates_SP(
		...
		SqlParametersMaps parameters = new SqlParametersMaps().put("oppkey",
				oppkey);
		result = executeStatement(connection, SQL_QUERY1, parameters, false)
				.getResultSets().next();
		...
		SqlParametersMaps parameters2 = new SqlParametersMaps().put("oppkey",
				oppkey).put("Language", "Language");
		result = executeStatement(connection, SQL_QUERY2, parameters2, false)
				.getResultSets().next();
		record = result.getCount() > 0 ? result.getRecords().next() : null;
		if (record != null) {
	...
	public Boolean IsSimulation_FN(SQLConnection connection, UUID oppkey)
		...
		SqlParametersMaps parameters = new SqlParametersMaps().put("oppkey",
				oppkey);
		SingleDataResultSet result = executeStatement(connection, SQL_QUERY,
	...
	// From TDSCore_Dev_Session2012_Sandbox
	public Boolean _AA_IsSegmentSatisfied_FN(SQLConnection connection,
			UUID oppkey, Integer segment) throws ReturnStatusException {
		boolean res = false;

		// Determine if all required items have been selected for a segment of a
		// test opp (just selected, not necessarily administered or responded
		// to)
		Integer ftcnt = null;
		Integer opcnt = null;
		String segmentKey = null;
		...
		SqlParametersMaps parameters2 = new SqlParametersMaps()
				.put("oppkey", oppkey).put("segment", segment)
				.put("isFieldTest", 0);
		result = executeStatement(connection, SQL_QUERY2, parameters2, false)
				.getResultSets().next();
		record = result.getCount() > 0 ? result.getRecords().next() : null;
		...
		SqlParametersMaps parameters3 = new SqlParametersMaps()
				.put("oppkey", oppkey).put("segment", segment)
				.put("isFieldTest", 1);
		result = executeStatement(connection, SQL_QUERY3, parameters3, false)
				.getResultSets().next();
		record = result.getCount() > 0 ? result.getRecords().next() : null;
		if (record != null) {
		...
		if (DbComparator.containsIgnoreCase(algorithm, ADAPTIVE)
			...
			SqlParametersMaps parameters4 = new SqlParametersMaps().put(
					"oppkey", oppkey).put("segmentKey", segmentKey);
			result = executeStatement(connection, SQL_QUERY4, parameters4,
					false).getResultSets().next();
			record = result.getCount() > 0 ? result.getRecords().next() : null;
			if (record != null) {
				oppitems = record.<Long> get("count").intValue();
			}
			if (DbComparator.lessOrEqual(ftneed, ftcnt)
					|| DbComparator.isEqual(oppitems, 0)) {
				res = true;
			}
		} else if (DbComparator.isEqual(algorithm, FIXEDFORM)
				&& DbComparator.isEqual(opneed, opcnt + ftcnt)) {
			res = true;
		}
	...
	// From TDSCore_Dev_Session2012_Sandbox
	public void _AA_NextFixedformGroup_SP(SQLConnection connection,
			UUID oppkey, String segmentKey, String language,
			_Ref<String> groupIDValue, _Ref<String> blockIDValue)
			throws ReturnStatusException {
		Date starttime = _dateUtil.getDateWRetStatus(connection);
		...
		SqlParametersMaps parameters = new SqlParametersMaps().put("oppkey",
				oppkey).put("segmentKey", segmentKey);
		SingleDataResultSet result = executeStatement(connection, SQL_QUERY,
				parameters, false).getResultSets().next();
		DbResultRecord record = result.getCount() > 0 ? result.getRecords()
				.next() : null;
		...
		SqlParametersMaps parameters2 = new SqlParametersMaps().put("oppkey",
				oppkey).put("segmentPosition", segmentPosition);
		result = executeStatement(connection, SQL_QUERY2, parameters2, false)
				.getResultSets().next();
		record = result.getCount() > 0 ? result.getRecords().next() : null;
		if (record != null) {
		...
		SqlParametersMaps parameters3 = new SqlParametersMaps()
				.put("segmentKey", segmentKey).put("formKey", formKey)
				.put("relativePosition", relativePosition);
		String query = fixDataBaseNames(SQL_QUERY3);
		result = executeStatement(connection, query, parameters3, false)
				.getResultSets().next();
		record = result.getCount() > 0 ? result.getRecords().next() : null;
		if (record != null) {
		...
		_commonDll._LogDBLatency_SP(connection, "_AA_NextFixedformGroup",
				starttime, null, true, null, oppkey);
	}
	 ...
	 * 
	 * @see
	 * tds.dll.api.IItemSelectionDLL#_AA_NextFieldtestGroup_SP(AIR.Common.DB
	 * .SQLConnection, java.util.UUID, int, java.lang.String, java.lang.String,
	 * java.lang.String, AIR.Common.Helpers._Ref, AIR.Common.Helpers._Ref,
	 * boolean)
	 */
	@Override
	public void _AA_NextFieldtestGroup_SP(SQLConnection connection,
			UUID oppkey, int segmentPosition, String segmentKey,
			String segmentID, String language, _Ref<String> groupID,
			_Ref<String> blockID, boolean debug) throws ReturnStatusException {
		Date starttime = _dateUtil.getDateWRetStatus(connection);
		...
		parameters = new SqlParametersMaps().put("oppkey", oppkey).put(
		...
		if (debug) {
			_logger.debug("First position = " + firstPosition
		...
		parameters = new SqlParametersMaps().put("oppkey", oppkey)
				.put("segmentKey", segmentKey).put("language", language)
				.put("relativePosition", relativePosition);
		result = executeStatement(connection, SQL_QUERY, parameters, true)
				.getResultSets().next();
		record = result.getCount() > 0 ? result.getRecords().next() : null;
		if (record != null) {
		 ...
		 * -- If there are no items meeting the accommodation requirements, then
		 * delete the itemgroup from this opportunity
		 */
		DataBaseTable tbl = AF_GetItempool_FN(connection, oppkey, segmentKey,
				segmentID, true, groupID.get(), blockID.get());

		SQL_QUERY = "select * from ${tblName}";
		Map<String, String> par = new HashMap<String, String>();
		par.put("tblName", tbl.getTableName());

		result = executeStatement(connection, fixDataBaseNames(SQL_QUERY, par),
		...
		if (DbComparator.isEqual(itemcnt, 0)) {
			...
			parameters = new SqlParametersMaps().put("oppkey", oppkey)
					.put("segmentKey", segmentKey)
					.put("groupID", groupID.get());
		...
		_commonDll._LogDBLatency_SP(connection, "_AA_NextFieldtestGroup",
				starttime, null, true, null, oppkey);
	}
	...
	public DataBaseTable AF_GetItempool_FN(SQLConnection connection,
		...
		int insertedCnt = executeStatement(connection,
				fixDataBaseNames(query, unquotedparms), parameters, true)
				.getUpdateCount();

		_logger.info("Inserted rows after Insert query in temporary table in AF_GetItempool_FN are "
				+ insertedCnt);
		...
		_logger.info("deleted rows after delete query in temporary table in AF_GetItempool_FN are "
				+ deletedCnt);
		...
		if (IsSimulation_FN(connection, oppkey)) {
			...
			_logger.info("updated rows after updated query in temporary table in AF_GetItempool_FN are "
					+ updatedCnt + " isSimulation = true");
		}
		...
		deletedCnt = executeStatement(connection,
				fixDataBaseNames(SQL_QUERY5, unquotedparms), parameters, true)
				.getUpdateCount();
		...
		if (fieldTest != null) {
			...
			deletedCnt = executeStatement(connection,
					fixDataBaseNames(SQL_QUERY6, unquotedparms), parameters,
					true).getUpdateCount();
	...
	// Optimal variant?
	public SingleDataResultSet AF_GetItempool_FN_2(SQLConnection connection,
			UUID oppkey, String segmentKey, String segmentID,
			Boolean fieldTest, String groupID, String blockID)
			throws ReturnStatusException {
		...
		UUID session = null;
		// Comment from BD:
		// -- declare @pool table (itemkey varchar(50) primary key not null,
		// propsin
		// int);
		SqlParametersMaps parameters;
		SingleDataResultSet result;
		...
		// Coment from BD: -- How many 'include' item properties are there for
		// this
		// test opp?
		SQL_QUERY = "select count(distinct propname) as propsin "
				+ " from testeeaccommodations A, ${ConfigDB}.client_test_itemconstraint C "
		...
		parameters = new SqlParametersMaps().put("oppkey", oppkey)
				.put("clientname", clientname).put("segmentID", segmentID);
		SQL_QUERY = fixDataBaseNames(SQL_QUERY);
		result = executeStatement(connection, SQL_QUERY, parameters, false)
				.getResultSets().next();
		record = result.getCount() > 0 ? result.getRecords().next() : null;
		...
		// Coment from BD:-- get all the items with their 'includes' property
		// count
		//
		// ==========================!!!!!!!!!!!!!!!!!======================================================================================================
		// new table pool will be have following columns and types:
		// itemkey, propsin, _fk_Item, groupKey, groupID, blockID, strand,
		// bVector,
		// IRT_model, IRT_a, IRT_b, IRT_c, itemPosition, isRequired,
		// isFieldTest,
		// isActive
		...
		parameters = new SqlParametersMaps().put("segmentKey", segmentKey)
				.put("groupID", groupID).put("blockID", blockID);
		SQL_QUERY = fixDataBaseNames(SQL_QUERY);
		pool = executeStatement(connection, SQL_QUERY, parameters, false)
				.getResultSets().next();
		// delete from SingleDataResultSet pool
		deleteFromItemPool(connection, pool, oppkey, clientname, segmentID,
				segmentKey);
		// update SingleDataResultSet pool
		updateItemPool(connection, pool, oppkey, clientname, segmentID,
				segmentKey);

		// Comment from BD:-- Join with item table to get active items with
		// required
		// number of properties and fieldtest criteria
		if (IsSimulation_FN(connection, oppkey))// Comment from BD: -- then this
												// is
												// a simulation, use
												// simulation's
	...
	protected void deleteFromItemPool(SQLConnection connection,
			SingleDataResultSet pool, UUID oppkey, String clientname,
			String segmentID, String segmentKey) throws ReturnStatusException {
		String itemKey = null;
	...
	// pool is updated SingleDataResultSet
	protected void updateItemPoolIfIsSimulation(SQLConnection connection,
			SingleDataResultSet pool, UUID session, String segmentKey)
			throws ReturnStatusException {// -- then this is a simulation, use
											// simulation's isrequired and
											// isactive
											// flags
		...
		parameters = new SqlParametersMaps().put("session", session).put(
				"segmentKey", segmentKey);
		res = executeStatement(connection, SQL_QUERY, parameters, false)
				.getResultSets().next();
		Iterator<DbResultRecord> poolItr = pool.getRecords();
		while (poolItr.hasNext()) {
			...
			while (tmpItr.hasNext()) {
				...
				if (poolRecord.<String> get("itemKey") == tmpRecord
						.<String> get("itemKey")) {
					poolRecord.addColumnValue("isRequired",
							tmpRecord.<Boolean> get("isRequired"));
					poolRecord.addColumnValue("isActive",
							tmpRecord.<Boolean> get("isActive"));
				}
			}
		}
	}
	...
	protected void deleteFromItemPool2(SingleDataResultSet pool, long propsin,
			String segmentKey, Boolean fieldTest) {
		DbResultRecord record;
		...
		while (poolItr.hasNext()) {
			...
			if (propsin < record.<Long> get("propsin")
					...
					|| (fieldTest != null && (fieldTest ^ record
							.<Boolean> get("isFieldTest")))) // TODO:
																// (AK)
																// check
																// it
			{
	...
	//
	public MultiDataResultSet AA_GetItemgroup_SP(SQLConnection connection,
			UUID oppkey, String segmentKey, String groupID, String blockID,
			Boolean isFieldTest, Boolean gebug) throws ReturnStatusException {
		...
		parameters.put("segmentKey", segmentKey).put("groupID", groupID)
				.put("blockID", blockID);
		result = executeStatement(connection, SQL_QUERY2, parameters, false)
				.getResultSets().next();
		record = result.getCount() > 0 ? result.getRecords().next() : null;
		if (record != null) {
			formKey = record.<String> get("formKey");
		...
		if (groupID != null && (groupID.startsWith(LIKESTRING_I)
						...
						// items for the simulation
			{
				...
				result = executeStatement(connection, SQL_QUERY4, parameters,
						false).getResultSets().next();
				record = result.getCount() > 0 ? result.getRecords().next()
						: null;
				if (record != null) {
			...
			res1 = executeStatement(connection, fixDataBaseNames(SQL_QUERY5),
					parameters, false).getResultSets().next();
		}
		...
		if (DbComparator.isEqual(algorithm, FIXEDFORM)) {
			// -- NOTE: Items will be positioned by form position by
			// T_InsertItems
			// -- Item filtering even by language is not applicable to fixed
			// form
			// tests.
			...
			res2 = executeStatement(connection, fixDataBaseNames(SQL_QUERY6),
					parameters, false).getResultSets().next();
			resultsets.add(res2);
		} else {
			String SQL_QUERY6 = "select _fk_Item as itemID, groupID, coalesce(${blockID}, 'NA') as blockID"
					...
					+ " from  ${tmpTableName} " + " order by itemPosition";
			parameters.put("blockID", blockID);
			...
			{
				tmpTable = _AA_GetCustomItemgroup_FN(connection, clientName,
						oppkey, segmentKey, testID, isFieldTest, groupID,
						blockID);
				unquotedParms1.put("tmpTableName", tmpTable.getTableName());
				res2 = executeStatement(connection,
						fixDataBaseNames(SQL_QUERY6, unquotedParms1),
						parameters, false).getResultSets().next();
				resultsets.add(res2);

			} else // Simulation
			{
				tmpTable = _AA_SIM_GetCustomItemgroup_FN(connection,
						clientName, oppkey, segmentKey, testID, isFieldTest,
						session, groupID, blockID);
				unquotedParms1.put("tmpTableName", tmpTable.getTableName());
				res2 = executeStatement(connection,
						fixDataBaseNames(SQL_QUERY6, unquotedParms1),
						parameters, false).getResultSets().next();
		...
		_commonDll._LogDBLatency_SP(connection, "AA_GetItemgroup", starttime,
				null, true, null, oppkey);
	 ...
	 * accommodations -- Get only the items that meet the testee's
	 * accommodations -- Every item must match all required constraint 'item_in'
	 * properties and not match any item_out properties. -- Constraints are
	 * actuated by the assignment to this test opp as a TesteeAccommodation. --
	 * There are only 2 types of constraints: Language, and TDSPoolFilter. The
	 * most common TDSPoolFilter constraints are by itemtype. -- Note that the
	 * group itself has already passed muster via _AA_GetCustomItempool
	 */
	public DataBaseTable _AA_GetCustomItemgroup_FN(SQLConnection connection,
			String clientname, UUID oppkey, String segmentKey, String testID,
			Boolean fieldTest, String groupID, String blockID)
			throws ReturnStatusException {
		DataBaseTable resDBTable = getDataBaseTable("tmpTable")
				...
				.addColumn("IRT_a", SQL_TYPE_To_JAVA_TYPE.FLOAT)
				.addColumn("IRT_b", SQL_TYPE_To_JAVA_TYPE.VARCHAR, 150)
				// TODO: (AK)
				// will be
				// changed with
				// AAlgorithm
		...
		final String SQL_INSERT = "INSERT into ${tblName} (itemkey, _fk_Item, groupKey, groupID, blockID"
				...
				+ " and P2.isactive = 1"
				+ ") "
				// -- each item must have the same number of records as the
				// number of
				// 'item_in' constraints
				+ " GROUP BY I._fk_ITem "
				+ ", I._fk_Item"
		...
		_logger.info("Insert count after Insert query in temporary table in _AA_GetCustomItemgroup_FN is "
				+ insertedCnt);
	...
	/**
	 * Comments from DB: -- Get custom item group as altered by simulation
	 * session -- Get all the items not administered on this test opp that meet
	 * the testee's accommodations -- Every item must match all required
	 * constraint 'item_in' properties and not match any item_out properties. --
	 * Constraints are actuated by the assignment to this test opp as a
	 * TesteeAccommodation. -- There are only 2 types of constraints: Language,
	 * and TDSPoolFilter. The most common TDSPoolFilter constraints are by
	 * itemtype.
	 */
	public DataBaseTable _AA_SIM_GetCustomItemgroup_FN(
			SQLConnection connection, String clientname, UUID oppkey,
			String segmentKey, String segmentID, Boolean fieldTest,
			UUID session, String groupID, String blockID)
			throws ReturnStatusException {
		DataBaseTable resDBTable = getDataBaseTable("tmpTable")
				.addColumn("itemkey", SQL_TYPE_To_JAVA_TYPE.VARCHAR, 150)
				.addColumn("_fk_Item", SQL_TYPE_To_JAVA_TYPE.VARCHAR, 150)
		...
		_logger.debug("Insert count after Insert query in _AA_SIM_GetCustomItemgroup_FN is "
				+ insertedCnt);
	 ...
	 * blueprint returned by AA_MakeCset1 -- Allows app-server-located
	 * computation of cset1 using cached segment blueprint -- Only needed for
	 * adaptive test segments. Fixed form segment selection is done by
	 * AA_GetNextItemCandidates using _AA_NextFixedFormGroup
	 * 
	 * -- TODO: Make a SIMULATOR VERSION that discriminates by session key
	 * 
	 * @throws ReturnStatusException
	 */
	public MultiDataResultSet AA_GetSegment_SP(SQLConnection connection,
		...
		res = executeStatement(connection, query, parameters, false)
				.getResultSets().next();
		record = res.getCount() > 0 ? res.getRecords().next() : null;
		if (record != null) {
		...
		SingleDataResultSet res1 = executeStatement(connection, query,
				parameters, true).getResultSets().next();
		...
		query = fixDataBaseNames(SQL_QUERY2);
		SingleDataResultSet res2 = executeStatement(connection, query,
				parameters, true).getResultSets().next();
		...
		query = fixDataBaseNames(SQL_QUERY3);
		SingleDataResultSet res3 = executeStatement(connection, query,
				parameters, true).getResultSets().next();
		...
		_commonDll._LogDBLatency_SP(connection, "AA_GetSegment", startTime,
				null, true, null, null, null, client, segmentKey);
	 ...
	 * @throws ReturnStatusException
	 */
	public MultiDataResultSet AA_SIM_GetSegment_SP(SQLConnection connection,
			UUID sessionKey, String segmentKey) throws ReturnStatusException {
		...
		SingleDataResultSet res1 = executeStatement(connection, SQL_QUERY1,
				parameters, true).getResultSets().next();
		resultsSets.add(res1);
		...
		_commonDll._LogDBLatency_SP(connection, "AA_SIM_GetSegment", startTime,
				null, true, null, null, sessionKey, client, segmentKey);
	...
	// testkey);
	// String query = fixDataBaseNames (SQL_QUERY);
	// result = executeStatement (connection, query, parameters,
	...
	// initialAbility)
	public MultiDataResultSet AA_GetDataHistory_SP(SQLConnection connection,
		...
		parameters.put("testee", testee).put("testID", testID)
				.put("clientname", clientname);
		res = executeStatement(connection, SQL_QUERY4, parameters, false)
				.getResultSets().next();
		if (res.getCount() > 0) {
			String itemgroupString = _commonDll.MakeItemGroupString_FN(
					connection, oppkey);
		...
		// -- Adaptive algorithm must not select item groups that have been
		// selected
		...
		_commonDll._LogDBLatency_SP(connection, "AA_GetDataHistory_SP",
				startTime, null, true, null, oppkey);
	 ...
	 * attempt to get the initial ability estimates for adaptive test -- this
	 * gets the most recently scored opportunity in this test, or another test
	 * of the same subject -- ASSUME: Previous year's scores are in an
	 * opportunity numbered 0 and are in a testID different from input
	 * parameter. -- 3/2011: Updated to utilize TestOpportunity._Key and to get
	 * clientname from TestOpportunity -- 9/2012: Convert the procedure to a
	 * function
	 */
	// Old variant
	public Float FN_GetInitialAbility_FN_2(SQLConnection connection, UUID oppkey)
			throws ReturnStatusException {
		...
		SqlParametersMaps parameters = new SqlParametersMaps().put("oppkey",
				oppkey).put("INITIALABILITY", INITIALABILITY);
		...
		if (record != null) {
			...
			try {
				...
				_logger.error("Cannot parse " + abilityString
						+ " from String to Float. Set ability = 0.0");
		...
		res = executeStatement(connection, SQL_QUERY1, parameters, false)
				.getResultSets().next();
		record = res.getCount() > 0 ? res.getRecords().next() : null;
		if (record != null) {
			clientname = record.<String> get("clientname");
		...
		tbl = executeStatement(connection, SQL_QUERY3, parameters, false)
				.getResultSets().next();
		...
		while (recItr.hasNext()) {
			...
			if (record != null) {
				if (test.equals(record.<String> get("test"))) {
					maxDateEqTest = new Date(Math.max(maxDateEqTest.getTime(),
							record.<Date> get("scored").getTime()));
				} else {
					maxDateNotEqTest = new Date(Math.max(
							maxDateNotEqTest.getTime(),
							record.<Date> get("scored").getTime()));
				}
		...
		if (maxdate.getTime() > 0) {
			...
			while (recItr.hasNext()) {
				record = recItr.next();
				if (record != null
						&& test.equals(record.<String> get("test"))
						&& record.<Date> get("scored").getTime() == maxdate
								.getTime()) {
			...
			if (maxdate.getTime() > 0) {
				...
				while (recItr.hasNext()) {
					...
					if (record != null
							&& !test.equals(record.<String> get("test"))
							&& record.<Date> get("scored") == maxdate) {// print
																		// 'Not
																		// the
																		// same
																		// test'
	...
	public Float FN_GetInitialAbility_FN(SQLConnection connection, UUID oppkey)
		...
		parameters.put("testee", testee).put("subject", subject);
		int insertedCnt = executeStatement(connection,
				fixDataBaseNames(SQL_QUERY3, unquotedparms), parameters, false)
				.getUpdateCount();
		...
		if (maxdate != null) {
			...
			return ability;
		} else if (bySubject) // -- failing that, try for same subject/different
								// test
		{
			query = "select max(scored) as maxdate from ${tblName} where test <> ${test}";
	 ...
	 * the item selection processes to determine specific applicability of each
	 * item (isActive, previously administered, etc.) -- Since an itempool
	 * cannot be altered (by subtraction) this procedure only needs to be run at
	 * the start of a new test opp -- However, it can be run to add to an
	 * itempool while the test is in progress, though this is almost never
	 * required. -- The custom itempool is not required for fixed form tests.
	 * 
	 * @param connection
	 * @param oppkey
	 * @param segmentKey
	 ...
	 * @throws ReturnStatusException
	 */
	public String _AA_ItemPoolString_FN(SQLConnection connection, UUID oppkey,
			String segmentKey) throws ReturnStatusException {
		...
		parameters.put("segmentKey", segmentKey).put("clientname", clientname)
				.put("testID", testID);
		String query = fixDataBaseNames(SQL_QUERY2);
		SingleDataResultSet pool = executeStatement(connection, query,
				parameters, true).getResultSets().next();
	...
	public boolean AA_SetSegmentSatisfied_SP(SQLConnection connection,
			UUID oppkey, Integer segmentPosition, String termReason)
			throws ReturnStatusException {
		...
		int updatedCnt = executeStatement(connection, SQL_QUERY, parameters,
				true).getUpdateCount();
	...
	public MultiDataResultSet AA_GetSegment2_SP(SQLConnection connection,
		...
		final String SQL_QUERY3 = " select groupID as GID, _fk_Item as itemkey"
				...
				// +
				// ", upper(IRT_Model) as irtModel, IRT_b as irtB, IRT_a as irtA, IRT_c as irtC, bVector"
				// Related to main document I need to comment this field and
				// added to populate
				// +
				...
				+ ", upper(IRT_Model) as irtModel, IRT_b as irtB, IRT_a as irtA, IRT_c as irtC, bVector"
				+ ", clString" + " from ${ItemBankDB}.tblsetofadminitems   "
				+ " where _fk_AdminSubject = ${segmentKey}";
		...
		if (parentTest != null) {
			...
			parameters.put("parentTest", parentTest).put("selectionAlgorithm",
					selectionAlgorithm);
			query = fixDataBaseNames(SQL_QUERY4);
			SingleDataResultSet res4 = executeStatement(connection, query,
					parameters, true).getResultSets().next();
	...
	// Following Query is Older version of optimized query.
...
//      + " where S._Key = ${segmentKey} or S.VirtualTest = ${parentTest}";
  //--order by segmentPosition, itemkey, Dimension, parmnum
  //Optimized by Sai
  final String SQL_QUERY5 =  "select "
      +"    coalesce(S.TestPosition, bigtoint(1)) as segmentPosition, "
      ...
      +"    ip.parmvalue"
      +" from"
      +"    ${ItemBankDB}.tblsetofadminitems AI"
      +"        inner join"
      +"    ${ItemBankDB}.tblsetofadminsubjects S ON S._Key = AI._fk_AdminSubject"
      +"        inner join"
      +"    ${ItemBankDB}.itemscoredimension as D ON D._fk_AdminSubject = AI._fk_AdminSubject"
      +"        and D._fk_Item = AI._fk_Item"
      +"        inner join"
      +"    ${ItemBankDB}.measurementmodel m ON m.ModelNumber = D._fk_MeasurementModel"
      +"        inner join"
      +"    ${ItemBankDB}.measurementparameter p ON p._fk_measurementModel = m.ModelNumber"
      +"        inner join"
      +"    ${ItemBankDB}.itemmeasurementparameter ip ON ip._fk_ItemScoreDimension = D._Key"
      +"        and ip._fk_MeasurementParameter = p.parmnum"
      +" where"
      +"    S._Key = ${segmentKey}"
      +"        or S.VirtualTest = ${parentTest}";
		...
		if(controlTriples)
		{
			...
			final String SQL_QUERY6 = "select bpElementID, name, value "
					...
					+ " union"
					+ " select p.bpElementID, ${proficientTheta} as name, pl.ThetaLo as value" // TODO: cast(cast(pl.ThetaLo as decimal(38, 18)) as varchar(200))
			...
			query = fixDataBaseNames(SQL_QUERY6);
			SingleDataResultSet res6 = executeStatement(connection, query,
					parameters, true).getResultSets().next();
			resultsSets.add(res6);			
		}
		
		String TDSPoolFilter = "TDSPoolFilter";
		String OFFGRADE = "OFFGRADE%";
		
		final String SQL_QUERY7 = "select propvalue, count(*) as count "
				+ " from ${ItemBankDB}.tblitemprops"
				+ " where _fk_AdminSubject = ${segmentKey} and propname = ${TDSPoolFilter}"
				+ " and propvalue like ${OFFGRADE}"
				+ " group by propvalue";
		parameters.put("TDSPoolFilter", TDSPoolFilter).put("OFFGRADE", OFFGRADE);
		query = fixDataBaseNames(SQL_QUERY7);
		SingleDataResultSet res7 = executeStatement(connection, query,
				parameters, true).getResultSets().next();
		resultsSets.add(res7);			

		_commonDll._LogDBLatency_SP(connection, "AA_GetSegment2", startTime,
				null, true, null, null, null, client, segmentKey);
    ...
    parameters.put("maxitems", maxItems);    
		...
		// -- Content-level specs (strands, content-levels and affinity groups
		// together)
		...
		// Dimensions and corresponding IRT params for the new AA - all
		// segments, including the current
...
final String SQL_QUERY5 =  "select "
  ...
  +"        or S.VirtualTest = ${parentTest}";
		...
		if(controlTriples)
		{
			...
			final String SQL_QUERY6 = "select bpElementID, name, value "
					+ " from sim_itemselectionparameter where _fk_Session = ${sessionKey} "
					+ " and _fk_AdminSubject = ${segmentKey}"
					+ " union"
					+ " select p.bpElementID, ${proficientTheta} as name, pl.ThetaLo as value" // TODO: cast(cast(pl.ThetaLo as decimal(38, 18)) as varchar(200))
		...
		_commonDll._LogDBLatency_SP(connection, "AA_SIM_GetSegment2",
				startTime, null, true, null, null, sessionKey, client,
				segmentKey);
	...
	public MultiDataResultSet AA_GetDataHistory2_SP(SQLConnection connection,
		...
		_commonDll._LogDBLatency_SP(connection, "AA_GetDataHistory2_SP",
				startTime, null, true, null, oppkey);
	 ...
	 * for adaptive test -- this gets the most recently scored opportunity in
	 * this test, or another test of the same subject -- ASSUME: Previous year's
	 * scores are in an opportunity numbered 0 and are in a testID different
	 * from input parameter.
	 * 
	 * -- 3/2011: Updated to utilize TestOpportunity._Key and to get clientname
	 * from TestOpportunity -- 9/2012: Convert the procedure to a function --
	 * 11/2013: added ability to specify a testID from which to pull initial
	 * ability, current or previous years; overrides bySubject. -- Also renamed
	 * form FN_GetInitialAbility for use with new adaptive algorithm. --
	 * =============================================
	 */
	public Float _AA_GetInitialAbility_FN(SQLConnection connection, UUID oppkey)
		...
		// -- if a testID was provided, that will override bySubject; these
		...
		final String SQL_QUERY3 = "insert into ${tblName} (oppkey, test, opportunity, scored, ability)"
				...
				// -- find all other opportunity candidates, same testee, same
				// subject, scored and not deleted
				// -- AM: changed to same subject or configured testId, in case
				// the configured testID is not
				// -- of the same subject as the current test opp (which I doubt
		...
		unquotedparms.put("tblName", pool.getTableName());
		parameters.put("testee", testee).put("subject", subject)
				.put("initialAbilityTestId", initialAbilityTestId);
		int insertedCnt = executeStatement(connection,
				fixDataBaseNames(SQL_QUERY3, unquotedparms), parameters, false)
				.getUpdateCount();
		...
		// -- DIFF TEST...
		// -- failing that, if another testID was configured, look in a prior
		// year for an opp
		if (initialAbilityTestId != null) {
		...
		// -- no longer returning the default value from the itembank, because
		// the blueprint will already
		// -- be loaded with default start ability values for all segments (and
		// reporting categories for that matter).
		// -- Returning a token value to flag to the caller that there is no
		// start value for this student.
		// -- print 'DEFAULT';
		// -- else get the default for this test from the itembank
		// -- print 'Itembank';
		return (float) -9999;
	...
	public MultiDataResultSet AA_GetDataHistory_LA2_SP(
		...
		// -- BIG CHANGE: Counting on TesteeHistory for info about other tests
		// taken this student
		...
		// -- placeholder for backward compatibility from the days when field
		// test items were preselected at test creation time
	...
	public MultiDataResultSet AA_GetDataHistory2_LA2_SP(
			...
			throws ReturnStatusException {
		Date startTime = _dateUtil.getDateWRetStatus(connection);
		// -- GEO NOTE: Startability is computed on the SHARD when the test opp
		// record is first generated. Do not attempt to compute here (on
		// satellite)
		String clientname = null;
		String itempool = null;
		String algorithm = null;
	...
	public SingleDataResultSet AA_AddOffgradeItems_SP(SQLConnection connection,
		...
		parameters.put("accType", accType).put("accCode1", accCode1);

		SingleDataResultSet res = executeStatement(connection, query, parameters, true).getResultSets().next();

		if (res.getCount() > 0) {
			parameters.put("status", "success").put("reason", "alredy set");
			outRes = executeStatement(connection, outQuery, parameters, true).getResultSets().next();
			return outRes;
		}

		final String query2 = "select * from testeeaccommodations "
				+ " where _fk_TestOpportunity = ${oppkey} and AccType = ${accType} and AccCode = ${accCode2}";
		parameters.put("accCode2", accCode2);

		res = executeStatement(connection, query2, parameters, true).getResultSets().next();

		if (res.getCount() == 0) {
			parameters.put("status", "failed").put("reason", "offgrade accommodation not exists");
			outRes = executeStatement(connection, outQuery, parameters, true).getResultSets().next();
			return outRes;
		}
		
		final String query3 = "update testeeaccommodations set AccCode = ${accCode1}, AccValue = ${accCode1} "
				+ " where _fk_TestOpportunity = ${oppkey} and AccType = ${accType} and AccCode = ${accCode2} ";
		
		int updateCnt = executeStatement(connection, query3, parameters, true).getUpdateCount();
		...
		parameters.put("adaptive", "adaptive%").put("segmentkey", segmentkey);
		
		res = executeStatement(connection, query4, parameters, true).getResultSets().next();
		
		DbResultRecord record;
		Iterator<DbResultRecord> resItr = res.getRecords();
		parameters.put("poolfilterProperty", poolfilterProperty);
		while (resItr.hasNext()) {
			record = resItr.next();
