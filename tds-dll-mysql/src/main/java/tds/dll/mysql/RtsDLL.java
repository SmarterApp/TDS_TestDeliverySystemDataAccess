...
public class RtsDLL extends AbstractDLL implements IRtsDLL
{
  ...
  public void _GetRTSAttribute_SP (SQLConnection connection, String clientname, Long testee, String attname, _Ref<String> attValue, Boolean debug) throws ReturnStatusException {
    ...
    if (DbComparator.isEqual (attname, "--ACCOMMODATIONS--")) {
      ...
      final String SQL_QUERY1 = "select ${rts}.tds_accommodationsstring(${testee}) as attrValue";
      sql_query = SQL_QUERY1;
      parameters = parameters1;

    } else if (DbComparator.isEqual (attname, "--ENTITYNAME--")) {
      final String SQL_QUERY2 = "select ${rts}._fentityname(${testee}) as attrValue";
      sql_query = SQL_QUERY2;
      parameters = parameters1;

    } else if (DbComparator.isEqual (attname, "--ETHNICITY--")) {
      final String SQL_QUERY3 = "select ${rts}.tds_fmapethnicity(${testee}, ${clientname}) as attrValue";
      sql_query = SQL_QUERY3;
      parameters = parameters2;

    } else if (DbComparator.isEqual (attname, "--ELIGIBLETESTS--")) {
      final String SQL_QUERY4 = "select ${rts}.tds_eligibletests(${testee}, ${clientname}) as attrValue";
      sql_query = SQL_QUERY4;
      parameters = parameters2;

    } else {
      final String SQL_QUERY5 = "select ${rts}.tds_fgetentityattribute(${testee}, ${attname}) as attrValue";
  ...
  public void _GetRTSEntity_SP (SQLConnection connection, String clientname, String externaId, String entityType, _Ref<Long> entityKeyRef) throws ReturnStatusException {
    ...
    if (rts == null)
      return;
    final String SQL_QUERY = "select ${rts}.tds_fgetentitykey(${externalId}, ${entityType}) as entityKey"; 
    //final String SQL_QUERY = "select  entityKey from ${rts}.tds_fgetentitykey(${externalId}) where EntityType = ${entityType} limit 1";
  ...
  public SingleDataResultSet _GetTesteeRelationships_SP (SQLConnection connection, String clientname, Long testee) throws ReturnStatusException {
    ...
    while (records.hasNext ()) {
      ...
      final String SQL_QUERY3 = "call ${rts}.tds_fgetparentkeys(${childkey}, ${relationship})";
      //final String SQL_QUERY3 = "select parentKey from ${rts}.tds_fgetparentkeys(${childkey}, ${relationship})";
  ...
  public void _ValidateInstitutionMatch_SP (SQLConnection connection, String clientname, Long testeeKey, Long proctorKey, _Ref<String> instKey) throws ReturnStatusException {
    ...
    final String cmd1 = "select  ${databaseRTS}._fparentkeys(${testeeKey})";
    Map<String, String> databaseNames1 = new HashMap<String, String> ();
    databaseNames1.put ("databaseRTS", rts);   
    SqlParametersMaps parameters1 = (new SqlParametersMaps ()).put ("testeeKey", testeeKey);
    //output will be written to temp table tblout_fparentkeys
    executeStatement (connection, fixDataBaseNames (cmd1, databaseNames1), parameters1, false);
    
    final String cmd2 = "select ${databaseRTS}.tds_userroles(${proctorKey})";
    Map<String, String> databaseNames2 = databaseNames1;
    SqlParametersMaps parameters2 = (new SqlParametersMaps ()).put ("proctorKey", proctorKey);
    //output will be written to temp table tblout_tds_userroles
    executeStatement (connection, fixDataBaseNames (cmd2, databaseNames2), parameters2, false);
    
    final String RTS_QUERY = "select  S.entityKey  as instKey from ${databaseRTS}.tblout_tds_userroles  R, "
        + " ${ConfigDB}.client_rtsroles C, ${databaseRTS}.tblout_fparentkeys S  where  R.RoleEntity = S.EntityKey "
        + " and  C.clientname = ${clientname} and R.role = C.RTS_Role and C.TDS_Role = 'proctor' limit 1";
    Map<String, String> databaseNames = databaseNames1; 
    
    SqlParametersMaps parameters = (new SqlParametersMaps ()).put ("clientname", clientname);
    
    String query  = fixDataBaseNames (RTS_QUERY);
    result = executeStatement (connection, fixDataBaseNames (query, databaseNames), parameters, false).getResultSets ().next ();
    DbResultRecord record = result.getCount () > 0 ? result.getRecords ().next () : null;
    if (record != null) {
  ...
  public void _GetRTSRelationship_SP (SQLConnection connection, String clientname, Long testee,
    ...
    //final String SQL_QUERY1 = "select RTSKey, ExternalID, EntityName from ${databaseRTS}.tds_fparentinstitution(${testee}, ${Rel})";
    final String SQL_QUERY1 = "call ${databaseRTS}.tds_fparentinstitution(${testee}, ${Rel})";
    ...
    @Override
    public SingleDataResultSet GetRTSUserRoles_SP(SQLConnection connection,
        String clientName, Long userKey) throws ReturnStatusException {
      // TODO Auto-generated method stub
      return null;
    }
    @Override
    public SingleDataResultSet GetRTSUserRoles_SP(SQLConnection connection,
        String clientName, Long userKey, Integer sessionType)
    ...
    public SingleDataResultSet R_GetSchoolGrades_SP (SQLConnection connection, String clientName, String schoolKey) throws ReturnStatusException {
      ...
      final String SQL_QUERY1 = "select SchoolKey from rtsschoolgrades where SchoolKey = ${schoolRTSKey} and (${expireDays} is null or datediff(dateEntered, ${starttime}) > ${expireDays}) limit 1";
    ...
    //@Override
    //public SingleDataResultSet R_GetSchoolGrades_SP (SQLConnection connection, long schoolKey) throws ReturnStatusException {
      // TODO Auto-generated method stub
    //  return null;
    //}
    ...
    public SingleDataResultSet getSchoolStudents (SQLConnection connection, String clientName, String schoolKey, String grade, String firstName, String lastName) throws ReturnStatusException {
      ...
      if (grade != null) {
        ...
        final String SQL_INSERT1 = "insert into ${StudentsTableName} (rtskey, SSID, LastName, FirstName, Grade) select entityKey, ExternalID, LastName, FirstName, Grade"
            + " from ${rts}.dbo.tds_fSchoolgradestudentswithextra(${school}, null, null, null)";
    ...
    public SingleDataResultSet GetRTSUser_SP (SQLConnection connection, String clientName, String userId) throws ReturnStatusException {
      ...
      final String SQL_QUERY = "select _Key as userkey, fullname, password, active, hasAcknowledged as ack from ${rts}.tbluser where username =  ${userID}";
     ...
     * @throws ReturnStatusException
     */
    //TODO not ported to mysql, still refers to table-valued function tds_userroles
    // We hacked it to do nothing
    public void _ValidateAsProctor_SP (SQLConnection connection, String clientName, Long userKey, _Ref<Long> userEntityRef, _Ref<Long> roleEntityRef, _Ref<String> roleRef, Integer sessionType)
      ...
      //final String SQL_INSERT = "insert into ${rolesTableName} (role, roleEntity, userEntity) select role, RoleEntity, UserEntity from  ${rts}.tds_userroles(${userkey});";
      ...
      //final String SQL_QUERY3 = "select R.role as role, R.userEntity as userEntity, R.roleEntity as roleEntity from ${ConfigDB}.client_rtsroles S, ${rolesTableName} R " +
     ...
     * @throws ReturnStatusException
     */
    //TODO not working version.We will be using hacked version 
    public SingleDataResultSet P_ValidateProctor_SPOrig (SQLConnection connection, String clientName, UUID browserKey, String proctorId, String password, Integer oppSessions, Boolean ignorepw,
      ...
      try {
        ...
        //TODO EF: after we create new tbluser in session db (see Oksana's email about it), we do this:
        //final String SQL_QUERY1 = "select _Key as userKey, fullname as fullname, password as rtspassword, entityKey as userEntity from tbluser where username = ${proctorID}" +
        //    " and (${ignorepw} = 1 or password = ${password}) ";
        final String SQL_QUERY1 = "select _Key as userKey, fullname as fullname, password as rtspassword from ${rts}.tbluser where username = ${proctorID}" +
        ...
        //TODO: EF  modify _ValidateAsProctor to do nothing, just not to break compilation
        _ValidateAsProctor_SP (connection, clientName, userKey.get (), userEntityRef, roleEntityRef, roleRef, sessionType);
        //take care here to not return
        if (userEntityRef.get () == null) {
          _commonDll._ReturnError_SP (connection, clientName, "P_ValidateProctor", "User does not have permission");
          _commonDll._LogDBLatency_SP (connection, "P_ValidateProctor", starttime, null, true, null, null, null, clientName, null);
          return result;
        }
  ...
  //TODO This is a hacked version
    public SingleDataResultSet P_ValidateProctor_SP (SQLConnection connection, String clientName, UUID browserKey, String proctorId, String password, Integer oppSessions, Boolean ignorepw,
      ...
      try {
        ...
        final String SQL_QUERY1 = "select userkey, fullname, rtspassword, entityKey " +
            " from tbluser where userid = ${proctorID}" +
            " and (${ignorepw} = 1 or rtspassword = ${password}) ";
        //final String SQL_QUERY1 = "select _Key as userKey, fullname as fullname, password as rtspassword from ${rts}.tbluser where username = ${proctorID}" +
        //    " and (${ignorepw} = 1 or password = ${password}) and active = 1 and hasAcknowledged = 1;";
        SqlParametersMaps parms1 = new SqlParametersMaps ().put ("proctorID", proctorId).put ("ignorepw", ignorepw).put ("password", password);
        Map<String, String> unquotedMap = new HashMap<String, String> ();
        ...
        //EF  ignore modified _ValidateAsProctor as it does nothing, just not to break compilation
        _ValidateAsProctor_SP (connection, clientName, userKey.get (), userEntityRef, roleEntityRef, roleRef, sessionType);
        // set userEntityKey to value we got from new session db tbluser table, rather than _ValidateAsProctor
