...
proc: begin
	...
	begin
		rollback;
		set v_msg = concat('excpetion handler: error at ', v_sofar);
	...
	drop temporary table if exists tmp_tblatts;
    create temporary table tmp_tblatts(
		tdsid varchar(50)
	  , attval varchar(500)
	);
	...
	start transaction;
		insert into tmp_tblatts (tdsid, attval)
		exec _gettesteeattributes v_clientname, v_testee;
		
		set v_sofar = 'first. ';

		insert into tmp_tblrels (reltype,  entitykey, tdsid, attval)
		exec _gettesteerelationships v_clientname, v_testee;
